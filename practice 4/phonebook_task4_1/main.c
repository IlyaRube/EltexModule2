...
static void print_contact_details(const Contact *contact, size_t index)
{
    ...
    print_string_list("Электронная почта", contact->emails,
                      contact->email_count);
...
static void print_contact_summary(const Contact *contact, size_t number)
{
    printf("%zu. ", number);
    print_contact_name(contact);

    if (contact->phone_count > 0U) {
...
static void list_contacts(const ContactBook *book)
{
    ...
    print_separator();
    printf("СПИСОК КОНТАКТОВ (%zu), СОРТИРОВКА ПО Ф.И.О.\n", book->count);
    print_separator();
    ...
    for (node = book->head; node != NULL; node = node->next) {
        print_contact_summary(&node->contact, number);
        number++;
    }
}

static void list_contacts_reverse(const ContactBook *book)
{
    const ContactNode *node;
    size_t number = book->count;

    print_separator();
    printf("КОНТАКТЫ В ОБРАТНОМ ПОРЯДКЕ (%zu)\n", book->count);
    print_separator();
...
static int read_required_field(const char *prompt,
                               char *field,
                               size_t field_size)
{
    for (;;) {
                            ...
                            size_t *count)
{
    ...
    *count = (size_t)requested_count;
    ...
    for (index = 0U; index < *count; index++) {
        snprintf(prompt, sizeof(prompt), "  Запись %zu: ", index + 1U);
        for (;;) {
            if (!input_read_line(prompt, items[index], MAX_ITEM_LENGTH)) {
                return 0;
            }
            if (items[index][0] != '\0') {
                break;
...
static int read_new_contact(Contact *contact)
{
    ...
    printf("После добавления контакт автоматически займёт место по Ф.И.О.\n\n");
                            ...
                            size_t *count)
{
    ...
    snprintf(prompt, sizeof(prompt),
             "Изменить раздел «%s»? (1 - да, 0 - нет): ", title);
...
static void edit_contact(ContactBook *book)
{
    ...
    printf("Если изменить Ф.И.О., узел будет перемещён в новое место списка.\n\n");
...
static void search_contacts(const ContactBook *book)
{
    ...
    print_separator();
    for (node = book->head; node != NULL; node = node->next) {
        if (contact_matches_query(&node->contact, query)) {
            printf("%zu. ", number);
            print_contact_name(&node->contact);
            printf("\n");
            found++;
        }
        number++;
...
static void print_menu(void)
{
    ...
    printf("7. Показать список в обратном порядке\n");
    printf("0. Выход\n");
    print_separator();
}
...
int main(void)
{
    ...
    for (;;) {
        print_menu();
        if (!input_read_int_range("Выберите пункт меню: ", 0, 7, &choice)) {
            printf("\nВвод завершён.\n");
            break;
        }
